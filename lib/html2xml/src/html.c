...
#define TAGLEVEL 128
#define TAGLENGTH 32
static char end_tag[TAGLEVEL][TAGLENGTH];
static int attribute[TAGLEVEL];
static int use_attribute_num = 0;
static FILE* xmlfile= NULL;

void xmlfile_set(FILE *file)
{
    if (file == NULL)
    {
        printf("xml file setting error \n");
        return;
    }
    xmlfile = file;
}
...
static void print_end_tag(int level)
{
    if(strlen(end_tag[level])!=0)
    {
        fprintf(xmlfile_get(),"</%s>\n",end_tag[level]);
        memset(end_tag[level],'\0',TAGLENGTH);
    }
}

static void set_end_tag(const char *tag, int level)
{
    int len = strlen(tag);

    if (tag==NULL)
    {
        printf("Error tag setting\n");
        return;
    }
    ...
    if (len < TAGLENGTH)
    {
        memcpy(end_tag[level],tag, len);
    }
    else
    {
        memcpy(end_tag[level],tag, TAGLENGTH);
    }
}

void attribute_reset(void)
{
    int i = 0;
    for (i = 0; i < use_attribute_num; i++)
    {
        attribute[i] = HTML_ATTRIB_UNKNOWN;
    }
    use_attribute_num = 0;
}

bool attribute_check(int key)
{
    int i = 0;
    if(use_attribute_num > 0)
    {
        for (i = 0; i < use_attribute_num ; i++)
        {
            if (attribute[i] == key)
    ...
    attribute[use_attribute_num++] = key;
...
#define INVAILD_CHARACTER "&"
void remove_invaild_xml_character(char *htmlstr)
{
    if (htmlstr == NULL)
    {
        return;
    ...
    char temp[1024];
    char *p=NULL;
    int len = strlen(htmlstr);

    memset(temp,'\0',1024);
    p = strtok(htmlstr,INVAILD_CHARACTER);
    while(p!=NULL)
    {
        strcat(temp,p);
        p = strtok(NULL,INVAILD_CHARACTER);
    };

    memcpy(htmlstr,temp,len);
...
void *html_print_dom_element(HtmlElement *element, int level){
	...
	while(element) {
        ...
        set_end_tag(html_tag[element->tag],level);
        //if (strlen(html_tag[element->tag]) != 0)
        {
		    if(element->text) 
            {
                remove_invaild_xml_character(element->text);
			    if(element->tag == HTML_TAG_NONE)
				    fprintf(xmlfile_get(),"%s\n", stringtrim_l(element->text));
			    else	
				    fprintf(xmlfile_get(),"<%s>\n%s\n", html_tag[element->tag], stringtrim_l(element->text));
		    }
            else if(strlen(html_tag[element->tag])!=0)
            {
			    attrib = element->attrib;
			    if (attrib) 
                {
                    attribute_reset();
                    fprintf(xmlfile_get(),"<%s ", html_tag[element->tag]);
				    do{
                        /*TODO: have bug in html parser, can't correct parsing*/
                        if(attribute_check(attrib->key))
                        {
                            remove_invaild_xml_character(attrib->value);
					        if (attrib->key == HTML_ATTRIB_UNKNOWN) {
						        //printf("%s=\"%s\" ", attrib->key_name, attrib->value);
					        } else {
						       fprintf(xmlfile_get(),"%s=\"%s\" ", html_attrib[attrib->key], attrib->value);
					        }
                        }
					    attrib = attrib->next;
				    }while(attrib);
